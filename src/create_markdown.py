...
with open('./src/Readme.md', 'w') as f:
    f.write(
        f"A curated list of papers on Large Language Models by year. I'll try to update the list if new papers are published. Let me know if I am missing important papers.\n\n")
    f.write(f"## TO-DO:\n\n")
    ...
    for year, papers in sorted(papers_by_year.items(), reverse=False):
        sorted_papers[year] = sorted(papers, key=lambda x: datetime.strptime(x['published_date'], '%d-%m-%Y'),
                                     reverse=False)
    for j, (year, papers) in enumerate(sorted_papers.items(), start=1):
            filename = f"./src/{year}.md"  # create a filename for each year
            with open(filename, "w") as file:
                ...
                for paper in papers:
                    ...
                    file.write(f"     {abstract}\n")
                    file.write(f"     {summarize(abstract)}\n")
                    i += 1
                    print(paper["title"])
